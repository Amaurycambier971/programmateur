...
while ws[f"{colone}{i}"].value != None :
    if "411" in str(ws[f"{colone}{i}"].value) :
...
pivot = pd.pivot_table(df, index=["ANNEE", "Num pièce","TIERS"], values=["Crédit","Débit"], aggfunc="sum")

writer = pd.ExcelWriter(file, engine='openpyxl', mode="a")
pivot = pivot.reindex(columns=["Débit","Crédit"])
...
# recreer le tableau (mise en forme)
for i in "ABCDE" :
    ws1[f"{i}1"].value = ws[f"{i}1"].value
